...
export default function FranchisePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await fetchJson<Collection>(`/api/tmdb/collection/${id}`, { cacheTtlMs: 3600000 });
        setCollection(data);
      } catch (err) {
    ...
    <div className="flex h-screen bg-background text-foreground font-sans overflow-hidden flex-col md:flex-row">
      ...
      <main className="flex-1 h-[100dvh] overflow-y-auto w-full custom-scrollbar relative">
        ...
        <CinematicHero
          ...
          theme="movie"
        >
          <div className="pb-6 sm:pb-12 px-4 sm:px-6 md:px-10 lg:px-12 xl:px-14 w-full flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        {id === "marvel" && (
          <div className="md:hidden w-full px-4 sm:px-6 pt-6 pb-2">
            <DoomsdayCountdown />
          </div>
        )}

        <div className="w-full px-4 sm:px-6 md:px-10 lg:px-12 xl:px-14 pb-14 pt-8">
            ...
            <div className="flex flex-col gap-12">
              {collection.groups.map((group, gIdx) => (
                <div key={gIdx}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 3xl:grid-cols-8 4xl:grid-cols-10 gap-x-4 gap-y-6 sm:gap-x-5 sm:gap-y-8">
