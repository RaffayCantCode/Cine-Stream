...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await tmdbFetch("/movie/popular", { page, include_adult: "false" }) as any;
    if (data?.results) data.results = excludeAnime(data.results);
    return Response.json(data, { headers: cacheHeaders(3600) });
  } catch (error) {
    return Response.json({ error: "Failed to fetch popular movies" }, { status: 500, headers: { "Cache-Control": "no-store, max-age=0" } });
  }
}
